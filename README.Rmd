...
# mosap: Multi-Objective Spatial Planning in R

<!-- badges: start -->
[![Lifecycle: experimental](https://img.shields.io/badge/lifecycle-experimental-orange.svg)](https://lifecycle.r-lib.org/articles/stages.html#experimental)
<!-- badges: end -->

`mosap` provides an exact optimisation framework for **multi-objective spatial planning** in problems where decisions are expressed as **management actions** applied across planning units.
...
`mosap` is designed for reproducible research workflows in conservation planning, restoration planning, and other spatial decision-support problems where actions, costs, ecological effects, and spatial structure interact.

## Experimental status

`mosap` is currently an experimental research package under active development.  
...
A typical `mosap` workflow has five steps:
...
data("sim_pu_sf", package = "mosap")
data("sim_features", package = "mosap")
data("sim_dist_features", package = "mosap")
```
...
A key idea in `mosap` is that objectives are registered as **atomic objectives** under user-defined aliases. These aliases can later be combined in multi-objective methods.
...
There are several ways to explore trade-offs in `mosap`. A simple option is to use a weighted-sum formulation.
...
For solved problems, `mosap` provides helper functions to extract user-facing summaries:
...
```{r}
plot_spatial(res, what = "pu")
plot_spatial(res, what = "actions")
```

## Why this example matters

This example illustrates the main modelling logic of `mosap`.
...
- the documentation of `inputData()`, `add_actions()`, `add_effects()`, and `solve()`,
