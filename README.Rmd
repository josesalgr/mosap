...
**⚠️ Experimental package: mosap is an experimental research package under active development.
Breaking changes may occur at any time (API, internal tables, solver interfaces, outputs).
Use it at your own risk and do not rely on it for production workflows yet.**

## Overview

The `mosap` is designed to support atomic objectives (e.g., minimize cost, minimize fragmentation, maximize benefits) and to combine them through multi-objective methods (e.g., weighted sum). The package supports both tabular and spatial inputs (vector- and raster-based workflows), and stores intermediate artifacts (tables, spatial relations, targets, solver configuration) to enable reproducible experimentation and reporting.
...
If you believe you've found a bug in mosap, please file an issue (ideally with a
reproducible example) at: <https://github.com/josesalgr/mosap/issues>.
