...
#'   \item \code{"actions"}: highlights selected actions within planning units.
#' }
#'
...
#' the number of distinct labels can grow quickly; in such cases, consider filtering
#' using \code{subset}, or ensure at most one action per PU.
#'
#' @details
...
#'
#' For \code{what = "actions"}, the optional \code{subset} argument can be used to restrict
#' the plotted actions. Values in \code{subset} may match:
#' \itemize{
#'   \item action ids in \code{x$Data$data$actions$id}, and/or
#'   \item action-group labels in \code{x$Data$data$actions$action_set} when available.
#' }
#'
#' Colors for \code{what = "actions"} are controlled by either:
#' \itemize{
#'   \item \code{fill_values}: a named vector mapping plotted labels to colors; or
...
#' @param subset Optional character vector. Only used when \code{what = "actions"}.
#'   Filters actions to a subset of action ids, \code{action_set} values, or both.
#' @param ... Additional arguments (reserved for future extensions).
#' @param base_alpha Numeric in \eqn{[0,1]}. Alpha used for the base PU layer.
#' @param selected_alpha Numeric in \eqn{[0,1]}. Alpha used for highlighted geometries.
...
#' @param fill_values Optional named character vector of colors. Names must match the plotted
#'   labels in the legend.
#' @param fill_na Color used for missing values in the manual scale.
...
#' plot(sol, what = "pu")
#'
#' # Plot all selected actions
#' plot(sol, what = "actions")
#'
#' # Plot only one action
#' plot(sol, what = "actions", subset = "harvest")
#'
#' # Plot one action group
#' plot(sol, what = "actions", subset = "restoration")
#'
#' # Plot a mix of action ids and groups
#' plot(sol, what = "actions", subset = c("harvest", "restoration"))
#' }
#'
...
plot.Solution <- function(
  ...
  if (is.null(pu_sf)) {
    stop("No PU geometry found to plot (need sf in x$Data$data$pu_sf or x$data$pu_sf).", call. = FALSE)
  }
  ...
  if (identical(what, "pu")) {
    ...
    if (!all(c("id", "selected") %in% names(pu_tbl))) {
      stop("PU table must contain 'id' and 'selected'.", call. = FALSE)
    }
    ...
    if (isTRUE(only_selected) && nrow(g_sel) == 0) {
  ...
  if (!is.null(subset)) {
    data_obj <- x$Data %||% x
    if (is.null(data_obj$data$actions) || !inherits(data_obj$data$actions, "data.frame")) {
      stop("Cannot use subset because actions metadata is missing in x$Data$data$actions.", call. = FALSE)
    }

    acts_keep <- .pa_resolve_action_subset(data_obj, subset = subset)
    keep_ids <- as.character(acts_keep$id)

    act_tbl <- act_tbl[act_tbl$action %in% keep_ids, , drop = FALSE]
  }

  if (nrow(act_tbl) == 0) {
    stop("No selected actions to plot for the requested subset.", call. = FALSE)
  }
  ...
  lab_act$action <- as.character(lab_act$action)
  ...
  if (!is.null(fill_values)) {
    if (is.null(names(fill_values)) || any(names(fill_values) == "")) {
      stop("fill_values must be a named vector with names matching plotted labels.", call. = FALSE)
    }
