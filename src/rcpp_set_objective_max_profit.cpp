...
// [[Rcpp::export]]
Rcpp::List rcpp_set_objective_max_profit(
    SEXP x,
  ...
  // -----------------------------
  if (op->_n_x <= 0) {
    // fallback if model didn't store it (not ideal, but defensive)
  ...
  for (int r = 0; r < n_da; ++r) {
    ...
    // keep first occurrence if duplicates exist (should not happen)
  ...
  // -----------------------------
  // registry: OBJECTIVE block on x range
  // -----------------------------
